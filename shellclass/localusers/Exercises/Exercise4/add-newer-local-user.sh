...
if [[ "$UID" -ne 0 ]]
then
    echo "You need to be the root user to run this script" 1>&2
    exit 1
fi
...
if [[ "$#" -lt 1 ]]
then
    echo "Usage: ${0} USER_NAME [COMMENTS]..." 1>&2
    exit 1
fi
...
then
    echo "There was an error while trying to execute useradd. Exit code ${USER_ADD_EXIT_CODE}" 1>&2
    exit 1
fi
